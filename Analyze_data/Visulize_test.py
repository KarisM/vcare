...
df = pd.read_csv('Sensor_data.csv',index_col = 0,names = ['dateTime','carId','sensorId','sensorName','value'])
df = df[0:]
...
--------------------------------------------------------------Sampling Data-----------------------------------------------------------------------
"""
def fuelToData(array):
    for idx in range(len(array)):
        array[idx,1] = (array[idx,1]/100.00) * 40.00
    return array
df_fuel = fuelToData(df_fuel)
...
def samplingData(array):
    data_list = []
    for idx in range(0,6*50,6):
        idx = len(array) - idx - 1
        avg = np.mean(array[idx-59:idx+1,1])
        data_list.append(int(avg))
    data_list.reverse()
#    print(data_list)
    return data_list

def getDiffInterval(array):
    data_list = []
    for idx in range(len(array)):
        if idx != len(array) - 1:
            diff = array[idx+1] - array[idx]
        data_list.append(abs(diff))
    data_list = data_list[:-1]
    return data_list

fuel = samplingData(df_fuel)   
distance = samplingData(df_distance)
    
diff_distance = getDiffInterval(distance)
diff_fuel = getDiffInterval(fuel)

def fuelUsage(distance,fuel):
    result = []
    for idx in range(len(distance)):
        if fuel[idx] == 0:
            ...
            res = float(distance[idx])/fuel[idx]
        result.append(res)
    return result

fuelConsumption = fuelUsage(diff_distance,diff_fuel)
